 ...
 * @return string
 */
function image_url_cache($source, $options = array()) {
  if (!$source) { return ''; }

  $options = _parse_attributes($options);

  if(!isset($options['width']) || !isset($options['height']) || !isset($options['section'])) {
    return '';
  }
  ...
  //przepisuje parametry do innej tablicy aby mozna bylo w opcjach przekazac inne parametry od image_tag
  $imageResizeParams = array(
    'file' => base64_encode($options['file']),
    'section' => base64_encode($options['section']),
    'width' => $options['width'],
    'height' => $options['height'],
    'mode' => $options['mode']
  );

  $routeName = "image_resize_mode";
  $genUrlParams = array_merge(array('sf_route' => $routeName), is_object($imageResizeParams) ? array('sf_subject' => $imageResizeParams) : $imageResizeParams);
  $url = sfContext::getInstance()->getController()->genUrl($genUrlParams);

  return $url;
}

function image_tag_cache($source, $options = array()) {
  $url = image_url_cache($source, $options);

  //wywalam z opcji parametry odnosnie generowania linku
  unset($options['file'], $options['section'], $options['mode'], $options['width'], $options['height']);
